...
public class SparkMaxStream {
  ...
  // Match all fields except API id and device id (first 10 bits)
  private static final int REV_API_MASK = 0x1FFFFC00;

  // Set default buffer size. Assume max telemetry frames = 8, max devices = 20
  // fastest rate = 5ms, polling rateworst case (with margin) = 40ms
  // (40 / 5) * 8 * 20 = 1280
  private static final int STEAM_BUFFER_SIZE = 1280;
   ...
   */
  public SparkMaxStream() {
    // All defaults, including reading from all devices. (Pass device arsing
    // logic down to callback...)
    m_stream = new TelemetryStream(REV_API_ID, REV_API_MASK, STEAM_BUFFER_SIZE);
  ...
  // Lazy way to test...
  protected void injectAndPoll(DataFrame[] frames, long timestamp, int canId) {
    ...
    for (int frameNum = 0; frameNum < frames.length; frameNum++) {
      ...
      messages[frameNum].messageID = frames[frameNum].arbId(canId);
